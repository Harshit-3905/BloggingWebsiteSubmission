...
// Finally load our custom styles
...
export function MarkdownRenderer({ content }: MarkdownRendererProps) {
  ...
  useEffect(() => {
    ...
    const copyButtons = containerRef.current.querySelectorAll(".copy-code-button");
  ...
  useEffect(() => {
    ...
    rendered = rendered.replace(/^#### (.*$)/gm, '<h4 class="text-lg font-bold my-2 text-foreground dark:text-foreground">$1</h4>');
    rendered = rendered.replace(/^##### (.*$)/gm, '<h5 class="text-base font-bold my-2 text-foreground dark:text-foreground">$1</h5>');
    rendered = rendered.replace(/^###### (.*$)/gm, '<h6 class="text-sm font-bold my-2 text-foreground dark:text-foreground">$1</h6>');
    ...
    rendered = rendered.replace(/^\* (.*)$/gm, '<li class="ml-6 list-disc">$1</li>');
    ...
    // Ordered lists
    // Convert list items first
    rendered = rendered.replace(/^\d+\. (.*)$/gm, '<li class="ml-6 list-decimal">$1</li>');
    ...
    rendered = rendered.replace(/<ul class="my-4 space-y-1">(<ul class="my-4 space-y-1">.*<\/ul>)<\/ul>/g, '$1');
    rendered = rendered.replace(/<ol class="my-4 space-y-1">(<ol class="my-4 space-y-1">.*<\/ol>)<\/ol>/g, '$1');
    ...
    if (renderedContent && containerRef.current) {
      try {
        ...
        setTimeout(() => {
          ...
          const codeElements = containerRef.current.querySelectorAll('code[class*="language-"]');
